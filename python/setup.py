...
class build_ext(_build_ext):
    ...
    def initialize_options(self):
        ...
        self.extra_cmake_args = os.environ.get('XYXLNTPYARROW_CMAKE_OPTIONS', '')
        self.build_type = os.environ.get('XYXLNTPYARROW_BUILD_TYPE', 'Debug')

        self.cmake_cxxflags = os.environ.get('XYXLNTPYARROW_CXXFLAGS', '')
    ...
    def _run_cmake(self):
        ...
        if sys.platform != 'win32':
            ...
            print("-- Runnning cmake for xyxlntpyarrow")
            self.spawn(cmake_command)
            print("-- Finished cmake for xyxlntpyarrow")
            args = ['make']
            if os.environ.get('XYXLNTPYARROW_BUILD_VERBOSE', '0') == '1':
                args.append('VERBOSE=1')

            if 'XYXLNTPYARROW_PARALLEL' in os.environ:
                args.append('-j{0}'.format(os.environ['XYXLNTPYARROW_PARALLEL']))

            args.append('install')

            print("-- Running cmake --build for xyxlntpyarrow")
            self.spawn(args)
            print("-- Finished cmake --build for xyxlntpyarrow")
        else:
            import shlex
            if not is_64_bit:
        ...
        try:
            os.makedirs(pjoin(build_lib, 'xyxlntpyarrow'))
        except OSError:
            pass
        ...
        if not os.path.exists(built_path):
            raise RuntimeError('xyxlntpyarrow C-extension failed to build:',
    ...
    def _get_cmake_ext_path(self, name):
        ...
        package_dir = build_py.get_package_dir('xyxlntpyarrow')
...
long_description = """
xyxlntpyarrow allows Apache Arrow tables to be written to and read from an XLSX
file efficiently using the C++ library xyxlnt.
""".strip()
