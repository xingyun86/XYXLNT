#ifndef XYXLNT_DETAIL_SERIALISATION_HELPERS_HPP
#define XYXLNT_DETAIL_SERIALISATION_HELPERS_HPP
...
///
/// a lightweight version of xyxlnt::cell_reference with no extre functionality (absolute/relative, ...)
/// many thousands are created during (de)serialisation, so even minor overhead is noticable
struct Cell_Reference
{
    // the obvious ctor
    explicit Cell_Reference(xyxlnt::row_t row_arg, xyxlnt::column_t::index_t column_arg) noexcept
    ...
    explicit Cell_Reference(xyxlnt::row_t row_arg, const std::string &reference) noexcept
        : row(row_arg)
    {
        ...
        column = static_cast<xyxlnt::column_t::index_t>(temp);
    }
    ...
    xyxlnt::row_t row; // range:[1, 1048576]
    xyxlnt::column_t::index_t column; // range:["A", "ZZZ"] -> [1, 26^3] -> [1, 17576]
};
...
struct Cell
{
    ...
    xyxlnt::cell_type type = xyxlnt::cell_type::number; // 't'
