...
public:
    ...
    static bool compare_relationships(const xyxlnt::manifest &left,
        const xyxlnt::manifest &right)
    {
        ...
        for (const auto &part : left.parts())
        {
            ...
            std::unordered_map<std::string, xyxlnt::relationship> left_rels_map;
    ...
    {
        xyxlnt::detail::vector_istreambuf left_buffer(left);
        std::istream left_stream(&left_buffer);
        xyxlnt::detail::izstream left_archive(left_stream);

        const auto left_info = left_archive.files();

        xyxlnt::detail::vector_istreambuf right_buffer(right);
        std::istream right_stream(&right_buffer);
        xyxlnt::detail::izstream right_archive(right_stream);
        ...
        {
            auto is_calc_chain = [](const xyxlnt::path &p) {
        ...
        xyxlnt::workbook left_workbook;
        left_workbook.load(left);

        xyxlnt::workbook right_workbook;
